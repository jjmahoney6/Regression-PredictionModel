...
# Load the CSV file
data = pd.read_csv(r"C:\Users\Busti\OneDrive - Worcester Polytechnic Institute (wpi.edu)\MQP\Sales Report Data Cleaning.csv", low_memory=False)
data = data[data['Name'] == 'Ticket Sales as of 9.15 Singles.xlsx']  # Filter for 'singles' only
...
for i in range(len(attendance_data)):
    plt.text(attendance_data.index[i], attendance_data['ATTENDANCE_RATE'].iloc[i] + 1,
             f'{attendance_data["ATTENDANCE_RATE"].iloc[i]:.1f}%', ha='center', va='bottom', fontsize=7)
...
for i in range(len(attendance_data)):
    plt.text(i - bar_width/2, attendance_data['total_tickets'].iloc[i] + 1,
             f'{attendance_data["total_tickets"].iloc[i]}', ha='center', va='bottom', fontsize=5)
    plt.text(i + bar_width/2, attendance_data['attended'].iloc[i] + 1,
             f'{attendance_data["attended"].iloc[i]}', ha='center', va='bottom', fontsize=5)
