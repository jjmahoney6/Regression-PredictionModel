...
bars_total = ax1.bar(positions, attendance_data['total_tickets'], width=bar_width, color='#f1948a', label='Total Tickets')
bars_attended = ax1.bar(positions, attendance_data['attended'], width=bar_width, color='#ab162b', label='Attended Tickets')

# Set x and y labels
ax1.set_xlabel('Ticket Paid Amount')
ax1.set_ylabel('Number of Tickets')
plt.title('Total Tickets and Attended Tickets by Ticket Paid Amount (2023)', color='black')
