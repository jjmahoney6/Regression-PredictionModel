...
# Add labels for total tickets and average tickets
for i, day in enumerate(days_of_week):
    # Total tickets label
    ax.text(x_positions[i] - 0.2, total_tickets_by_day[i] + 1, f"{total_tickets_by_day[i]}", 
            ha='center', va='bottom', fontsize=9, color='blue')
    # Average tickets label (rounded to integer)
    ax.text(x_positions[i] + 0.2, avg_tickets_by_day[i] + 1, f"{avg_tickets_by_day[i]}", 
